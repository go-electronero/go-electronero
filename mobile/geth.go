...
func NewNode(datadir string, config *NodeConfig) (stack *Node, _ error) {
	...
	if config.EthereumGenesis != "" {
		...
		// If we have the TestNero testnet, hard code the chain configs too
		...
		// If we have the NeroNet mainnet, hard code the chain configs too
		if config.EthereumGenesis == MainnetGenesis() {
			genesis.Config = params.MainnetChainConfig
			if config.EthereumNetworkID == 1 {
				config.EthereumNetworkID = 888
			}
		}
	}
